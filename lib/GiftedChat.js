...
function GiftedChat(props) {
    const { messages = [], initialText = "", isTyping, messageIdGenerator = () => uuidv4(), user = {}, onSend, locale = "en", renderLoading, actionSheet = null, textInputProps, renderChatFooter = null, renderInputToolbar = null, bottomOffset = 0, focusOnInputWhenOpeningKeyboard = true, keyboardShouldPersistTaps = Platform.select({
        ...
        android: "always",
        default: "never",
    }), onInputTextChanged = null, maxInputLength = null, inverted = true, minComposerHeight = MIN_COMPOSER_HEIGHT, maxComposerHeight = MAX_COMPOSER_HEIGHT, isKeyboardInternallyHandled = true, shouldStickMessageToTop = false, isStatusBarTranslucentAndroid, isNavigationBarTranslucentAndroid, } = props;
        ...
        transform: [
            {
                translateY: -(keyboard.height.value -
                    (keyboard.height.value > 120 && messages.length <= 2 ? 120 : 0)) + keyboardOffsetBottom.value,
            },
        ],
    ...
    const contentStyleAnim1 = useAnimatedStyle(() => ({
        transform: [
            {
                translateY: -(keyboard.height.value > 120 && messages.length <= 2
                    ? 120
                    : 0),
            },
        ],
      ...
      <ActionSheetProvider ref={actionSheetRef}>
        <View testID={TEST_ID.WRAPPER} style={[styles.fill, styles.contentContainer]} onLayout={onInitialLayoutViewLayout}>
          {isInitialized ? (<Animated.View style={[
              ...
              {shouldStickMessageToTop ? (<Animated.View style={[
                    styles.fill,
                    isKeyboardInternallyHandled && contentStyleAnim2,
                ]}>
                  {renderMessages}
                </Animated.View>) : (renderMessages)}
              <Animated.View style={[isKeyboardInternallyHandled && contentStyleAnim1]}>
